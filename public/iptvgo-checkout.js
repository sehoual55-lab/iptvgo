...
(function () {
  "use strict";
  if (window.__goCheckoutLoaded) return;
  window.__goCheckoutLoaded = true;

  var CONFIG = {
    ENDPOINT: "https://script.google.com/macros/s/AKfycbygzbbiXDXG8IdsT-Rg36VZNOOOtRQC6uw7VEvpfRXzsYj5RO2XdXtPjGPY_sXVSFn2/exec",
  ...
  // -------- i18n --------
  var LANGS = {
    en: { eyebrow:"IPTVGO Subscription", title:"Complete your order", sub:"Enter your details to activate your subscription.",
      ...
      msg:function(p,d,c,pr,n,m,l){return "Hi IPTVGO! I'd like to order the "+p+" ("+d+", "+c+") for "+pr+". Name: "+n+". Payment: "+m+(l?(". Payment link: "+l):".");} },
    fr: { eyebrow:"Abonnement IPTVGO", title:"Finalisez votre commande", sub:"Entrez vos informations pour activer votre abonnement.",
      ...
      msg:function(p,d,c,pr,n,m,l){return "Bonjour IPTVGO, je souhaite commander "+p+" ("+d+", "+c+") pour "+pr+". Nom : "+n+". Paiement : "+m+(l?(". Lien de paiement : "+l):".");} },
    it: { eyebrow:"Abbonamento IPTVGO", title:"Completa il tuo ordine", sub:"Inserisci i tuoi dati per attivare l'abbonamento.",
      ...
      msg:function(p,d,c,pr,n,m,l){return "Ciao IPTVGO! Vorrei ordinare "+p+" ("+d+", "+c+") per "+pr+". Nome: "+n+". Pagamento: "+m+(l?(". Link di pagamento: "+l):".");} }
  };
  function detectLang(){ var p=location.pathname; if(/^\/fr(\/|$)/.test(p))return "fr"; if(/^\/it(\/|$)/.test(p))return "it";
    var l=(document.documentElement.lang||"").slice(0,2); return LANGS[l]?l:"en"; }
  ...
  '.fs-sum{padding:30px 28px;background:radial-gradient(120% 70% at 6% 0%,rgba(25,217,192,.14),transparent 60%),#0a0c10;border-right:1px solid #171b22;display:flex;flex-direction:column}'+
  ...
  '.fs-pay:hover{border-color:#333b47}.fs-pay.sel{border-color:'+LIME+';background:rgba(25,217,192,.10);color:#fff}'+
  ...
  '.fs-check{width:66px;height:66px;border-radius:50%;background:rgba(25,217,192,.15);display:flex;align-items:center;justify-content:center;margin:0 auto 16px}'+
    ...
    modal.innerHTML=
      ...
      '<div class="fs-body" style="display:contents">'+
        '<aside class="fs-sum">'+
          ...
          '<div class="fs-badge" data-pop style="display:none">★ '+(LANG==="fr"?"Le Plus Populaire":LANG==="it"?"Più Popolare":"Most Popular")+'</div>'+
