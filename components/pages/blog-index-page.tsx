...
export function BlogIndexPage({ lang }: { lang: Locale }) {
  const dict = getDictionary(lang);
  const { eyebrow, title, subtitle, readMore, posts } = dict.blog;
              ...
              <Reveal key={post.slug} delay={i * 0.08}>
                <article className="group flex h-full flex-col overflow-hidden rounded-3xl glass transition-all duration-300 hover:-translate-y-1 hover:shadow-card-hover">
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    ...
                    <h2 className="mt-3 font-display text-lg font-semibold leading-snug text-mist transition-colors group-hover:text-teal-300">
                      {post.title}
                    </h2>
